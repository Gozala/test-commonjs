...
function AssertionError(options) {
  ...
  // V8 specific
  if (Error.captureStackTrace) Error.captureStackTrace(this, fail)
}
...
, raises: function raises(block, Error, message) {
    var threw = false,
        exception = null;
    // (block)
    ...
    if (threw && (!Error || exception instanceof Error)) this.pass(message)
    else this.fail(
      { message: message
      , operator: "throw"
      })
